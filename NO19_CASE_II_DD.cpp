...
template<class F> void eachSing(int type,F f){ // type 1 Q, 2 D
 const int block[10][2]={{1,8},{0,-1},{2,-1},{3,-1},{4,-1},{5,-1},{6,-1},{7,-1},{9,-1},{10,-1}}; const int bs[10]={2,1,1,1,1,1,1,1,1,1}; int special=(type==2?0:1);
...
std::vector<T> makeAreps(){
 std::unordered_set<uint64_t> pool;pool.reserve(600000);eachSing(2,[&](const T&t){pool.insert(enc(t));});
 std::vector<std::array<int,11>> gs;int in[6]={2,3,4,5,6,7};do{int out[2]={9,10};do{std::array<int,11>p{};for(int i=0;i<11;i++)p[i]=i;for(int i=0;i<6;i++)p[2+i]=in[i];p[9]=out[0];p[10]=out[1];gs.push_back(p);}while(std::next_permutation(out,out+2));}while(std::next_permutation(in,in+6));
...
int main(){auto start=std::chrono::steady_clock::now();
 ...
 std::vector<T>B;B.reserve(449280);eachSing(2,[&](const T&t){B.push_back(t);});std::cerr<<"B_candidates="<<B.size()<<"\n";
